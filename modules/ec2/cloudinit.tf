...
data "cloudinit_config" "ubuntu" {
  ...
  part {
    ...
    content_type = "text/x-shellscript"
    content      = file("${path.module}/config/bootstrap-ubuntu.sh")
  }

  part {
    filename     = "bootstrap.yaml"
    content_type = "text/cloud-config"
    content      = file("${path.module}/config/cloud-config-ubuntu.yaml")
  }
}
...
data "cloudinit_config" "freebsd" {
  ...
  part {
    ...
    content_type = "text/x-shellscript"
    content      = file("${path.module}/config/bootstrap-freebsd.sh")
  }

  part {
    filename     = "bootstrap.yaml"
    content_type = "text/cloud-config"
    content      = file("${path.module}/config/cloud-config-freebsd.yaml")
  }
}
...
data "cloudinit_config" "flatcar" {
  ...
  part {
    ...
    content_type = "text/x-shellscript"
    content      = file("${path.module}/config/bootstrap-flatcar.sh")
  }

  part {
    filename     = "bootstrap.yaml"
    content_type = "text/cloud-config"
    content      = file("${path.module}/config/cloud-config-flatcar.yaml")
  }
}
